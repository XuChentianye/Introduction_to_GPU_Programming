...
int main(int argc, char *argv[])
{
    ...
    //ToDo
    printf("\nDecrypting using GPU...\n");
    int memSize=lenStr*sizeof(char);
    char *d_A=NULL;
    char *d_B=NULL;
    char *h_decryptedStr_from_gpu=NULL;
    h_decryptedStr_from_gpu=(char *)malloc(memSize);
    ...
    decrypt_gpu<<<lenStr,1>>>(d_A, d_B, lenStr, pwd);
    CHECK(cudaMemcpy(h_decryptedStr_from_gpu, d_B, memSize, cudaMemcpyDeviceToHost));

    // Write to output file
    WriteFile(output_file, h_decryptedStr_from_gpu, read_size);
    printf("\nDecrypted message has been saved to %s ...\n", output_file);
    CHECK(cudaFree(d_A));
    CHECK(cudaFree(d_B));
    free(inputStr);
    free(h_decryptedStr_from_gpu);
