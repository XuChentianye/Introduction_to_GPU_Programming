...
__device__ float filter_glo[GAUSSIAN_SIZE];
...
void gaussian_filter(const float mu, const float sigma)
{
	...
	cudaMemcpyToSymbol(filter_glo, filter, GAUSSIAN_SIZE*sizeof(float));
	free(filter);
}
...
int main(int argc, char *argv[])
{
	...
	cudaMemcpyFromSymbol(filter1, filter_glo, GAUSSIAN_SIZE*sizeof(float));
